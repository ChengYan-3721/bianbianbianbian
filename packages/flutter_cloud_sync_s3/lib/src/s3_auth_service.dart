...
///
/// 用户身份策略（决定 [getCurrentUser] 返回的 [CloudUser.id]，进而决定
/// `flutter_cloud_sync` 的远端文件夹名 `users/<userId>/...`）：
/// - 优先使用调用方传入的 [customName]——非空时直接作为 id；
/// - 否则回退到 `s3-${accessKey}`，保留 Access Key 派生的兼容行为。
class S3AuthService implements CloudAuthService {
  final S3Client client;
  final String bucket;
  final String? customName;

  S3AuthService(this.client, this.bucket, {this.customName});

  String get _userId {
    final n = customName?.trim();
    if (n != null && n.isNotEmpty) return n;
    return 's3-${client.accessKey}';
  }
