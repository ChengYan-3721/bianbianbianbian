...
class CloudServiceConfig {
  ...
  /// 用户自定义昵称——所有可配置后端共享的"易记标题"。
  /// - 非空时，[obfuscatedUrl] 直接返回它（替代各后端默认的 host / endpoint
  ///   拼接），驱动 UI 卡片标题；
  /// - **额外副作用仅 S3**：S3AuthService 会用它作为 `CloudUser.id`，进而
  ///   把云端文件夹路径前缀从 `users/s3-<accessKey>/...` 改为
  ///   `users/<customName>/...`。Supabase 受 RLS 约束（`users/<auth.uid()>/...`）
  ///   不能动路径；WebDAV 路径已由 username 决定，也保持不变——这两端
  ///   customName 仅影响标题。
